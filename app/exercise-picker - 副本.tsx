      ...
      <View style={styles.bodyRow}>
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            const fg = active ? '#FFFFFF' : colors.primary;
            const chev = active ? 'rgba(255,255,255,0.9)' : colors.textTertiary;
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <MuscleIcon cat={item} color={fg} size={20} />
                <Text style={[styles.sideText, { color: active ? '#FFF' : colors.text }]} numberOfLines={1}>
                  {item}
                </Text>
                <View style={[styles.sideCount, { backgroundColor: active ? 'rgba(255,255,255,0.95)' : colors.primarySoft }]}>
                  <Text style={[styles.sideCountText, { color: active ? colors.primary : colors.primary }]}>{count}</Text>
                </View>
                <Ionicons name="chevron-forward" size={14} color={chev} />
              </TouchableOpacity>
            );
          }}
        />
...
const styles = StyleSheet.create({
  ...
  sideText: { flex: 1, fontSize: FontSize.sm, fontWeight: '600' },
  sideCount: {
    minWidth: 22, height: 16, paddingHorizontal: 5,
    borderRadius: BorderRadius.full, alignItems: 'center', justifyContent: 'center',
  },
  sideCountText: { fontSize: 9, fontWeight: '700' },

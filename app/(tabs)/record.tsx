...
export default function RecordScreen() {
  ...
  const [availableTags, setAvailableTags] = useState(DefaultTags.map(t => t.name));
            ...
            <View>
              ...
              {sportList.length === 0 ? (
                <View style={styles.sportEmpty}>
                  <Ionicons name="fitness-outline" size={26} color={colors.textTertiary} />
            ...
            <View>
              ...
              <View style={styles.starRow}>
                {[1, 2, 3, 4, 5].map(score => (
                  <TouchableOpacity key={score} onPress={() => setSleepScore(sleepScore === score ? 0 : score)}>
                    <Ionicons name={score <= sleepScore ? 'star' : 'star-outline'} size={28} color={score <= sleepScore ? colors.warning : colors.textTertiary} />
                  </TouchableOpacity>
                ))}
              </View>
              <Text style={[styles.subLabel, { color: colors.textSecondary }]}>身体状态 (多选)</Text>
              <View style={styles.tagsContainer}>
                {BodyStatusOptions.map(bs => (
              ...
              <View style={styles.menstrualRow}>
                <TouchableOpacity style={[styles.toggleRow, { borderColor: colors.border }]} onPress={() => setIsMenstrual(!isMenstrual)} activeOpacity={0.7}>
                  <Ionicons name={isMenstrual ? 'toggle' : 'toggle-outline'} size={28} color={isMenstrual ? colors.primary : colors.textTertiary} />
